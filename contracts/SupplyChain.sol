...
contract SupplyChain {
  ...
  modifier isOwner(address _address){
    require(owner == _address, 'Can only be called by contract owner');
    _;
  }
  ...
  modifier shipped(uint _sku){
    require(items[_sku].state == State.Shipped, 'Item was not shipped');
    _;
  }
  ...
  constructor() {    // Note: public removed because of message "Visibility for constructor is ignored. If you want the contract to be non-deployable, making it abstract is sufficient"
    // 1. Set the owner to the transaction sender
    owner = msg.sender;
    // 2. Initialize the sku count to 0. Question, is this necessary?
    skuCount = 0;
    // No, because it's initialized to 0 by default.
  }
